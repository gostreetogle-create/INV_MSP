...
### Решено: жёсткая привязка distance экструзии к параметру

Раньше не получалось (`extrude.ExtentOne.Distance.Expression = "..."` → `AttributeError` —
свойства `ExtentOne` там просто нет). Рабочий путь найден: **`extrude.Extent.Distance`** (без
"One"), а не `ExtentOne.Distance`:

```python
extrude = compDef.Features.ExtrudeFeatures.Add(extrudeDef)
extrude.Extent.Distance.Expression = "Толщина"   # теперь живая ссылка на параметр, не число
```

Проверено: после этого изменение параметра "Толщина"/"Длина" в UserParameters реально
пересчитывает объём тела (не только 2D-эскиз, как было раньше). `core.py` обновлён — оба
`create_sheet_part` и `create_tube_part` используют это.
...
## Аудит: импорт блоков AutoCAD с сохранением повторного использования (2026, не реализовано)

**Задача**: при переносе 3D-модели из AutoCAD в Inventor, если в AutoCAD одинаковые детали
оформлены как блоки (block references), хотелось бы получить в Inventor сборку, где эти
повторяющиеся детали — это **несколько экземпляров одной и той же детали**, а не N визуально
похожих, но независимых деталей (что сейчас и происходит через обычный импорт).

**Вывод после исследования**: это реальное, известное ограничение интеропа AutoCAD↔Inventor, а
не то, что мы что-то не знаем/не умеем.

- То, что в справке Inventor называется «AutoCAD Blocks» — про **2D-символы в среде чертежа**
  (Drawing Resources), не про 3D-геометрию и не про сборки. Не то, что нужно.
- Штатный переводчик 3D DWG/AutoCAD-файлов в Inventor разворачивает геометрию в независимые тела
  без сохранения информации «это повторное использование одного блока» — подтверждено несколькими
  независимыми источниками, включая форум Autodesk.
- API Inventor **не имеет** документированного пути для программной вставки ACAD/DWG с
  сохранением block-структуры — это явное ограничение, не просто нехватка примера кода.
- Известные обходные пути в сообществе идут в **обратную** сторону от того, что нужно нам
  (например, "разбить блок на солиды и объединить в один solid перед импортом" — это про
  превращение блока в цельное тело, а не про сохранение его как переиспользуемого компонента).

**Рекомендация**: не браться за это сейчас. Если понадобится в будущем — вероятный путь не через
штатный импорт, а через **AutoCAD-сторону**: скриптом в AutoCAD (AutoLISP или COM) выгрузить
каждое уникальное определение блока в отдельный файл (один файл на уникальную геометрию, не на
каждый экземпляр) плюс список позиций/трансформаций всех вхождений — и уже эти данные скормить
нашему `core.py` (`open_document` + `Occurrences.Add` + `Transformation` — эта часть уже
проверена и работает, см. раздел «Сборки» выше) для сборки в Inventor с настоящим повторным
использованием детали. Это отдельная, небольшая, но реальная разработка, требующая: (1) умения
автоматизировать саму AutoCAD, (2) реальных файлов заказчика с блоками для проверки на практике,
прежде чем обещать результат. Оставляю как задачу на будущее, не блокирует текущую работу.
...
## Соглашение: имена параметров, которые создаём сами — на русском

По требованию пользователя все User Parameters, создаваемые нашими функциями (`core.py`),
называются по-русски: `Длина`, `Ширина`, `Толщина`, `ШиринаНаружная`, `ВысотаНаружная`,
`ТолщинаСтенки`. Кириллица в именах параметров и в выражениях (`"ШиринаНаружная - 2 *
ТолщинаСтенки"`) поддерживается Inventor API без ограничений — проверено на создании,
привязке к размерам эскиза и живом пересчёте. Автоматически генерируемые Inventor'ом системные
параметры (`d0`, `d1`, …) остаются как есть — их имена не контролируются нами.

## ⚠️ Грабли: `Document.Close(SkipSave)` — перепутанная логика булева параметра

Много раз за эту сессию скрипт зависал на 120 секунд без единой строки вывода — причина каждый
раз одна и та же: где-то раньше вызвался `doc.Close(False)` на документе с несохранёнными
изменениями, и Inventor показал модальный диалог **"Сохранить изменения?"** (Да/Нет/Отмена),
который блокирует ВСЕ последующие COM-вызовы, пока его не закрыть руками (диалог называется
"Autodesk Inventor Professional", без версии/имени файла в заголовке — так его и находить через
`EnumWindows`).

- `doc.Close(True)` — закрыть **без** сохранения и **без** диалога. Безопасно для тестовых/
  выброс-документов.
- `doc.Close(False)` — закрыть **с попыткой** сохранить (или спросить, если непонятно) — здесь и
  ловим зависание.

**Правило: для любых временных/тестовых документов всегда `Close(True)`, никогда `Close(False)`.**
Если зависание всё же случилось — найти окно `"Autodesk Inventor Professional"` (без "2027" в
заголовке) и кликнуть "Нет" по координатам (~76%, ~72% от размера окна — там всегда третья кнопка
"Нет" при стандартной раскладке Да/Нет/Отмена).
